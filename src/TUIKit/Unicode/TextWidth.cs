namespace TUIKit.Unicode
{
    ...
    /// <summary>
    /// Computes the terminal column width of Unicode scalar values using self-contained interval
    /// tables. The implementation follows the widely used <c>wcwidth</c> model (Markus Kuhn) with
    /// added ranges for modern emoji, so results are identical on every target framework rather
    /// than depending on the host runtime's Unicode data. All members are thread-safe.
    /// </summary>
    public static class TextWidth
    {
        // Zero-width scalars: combining marks, default-ignorable code points, and format controls.
        // Stored as a flat sorted array of [lo, hi] pairs.
        private static readonly int[] _ZeroWidth =
        {
            ...
            0x112DF, 0x112DF, 0x112E3, 0x112EA, 0x11301, 0x11301, 0x1133C, 0x1133C,
            ...
            0x116B7, 0x116B7, 0x16AF0, 0x16AF4, 0x16B30, 0x16B36, 0x16F8F, 0x16F92,
        ...
        // Wide (2-column) scalar ranges: East Asian Wide/Fullwidth plus common emoji blocks.
        private static readonly int[] _Wide =
        {
            ...
            0xFFE0, 0xFFE6, 0x16FE0, 0x16FE1, 0x17000, 0x187EC, 0x18800, 0x18AF2,
            ...
            0x1F250, 0x1F251, 0x1F300, 0x1F64F, 0x1F680, 0x1F6FF, 0x1F900, 0x1F9FF,
            0x1FA70, 0x1FAFF, 0x20000, 0x2FFFD, 0x30000, 0x3FFFD
        };

        // Extended pictographic emoji ranges, used by grapheme segmentation and to force
        // emoji-presentation clusters to width 2.
        private static readonly int[] _Pictographic =
        {
            0x00A9, 0x00A9, 0x00AE, 0x00AE, 0x203C, 0x203C, 0x2049, 0x2049,
            0x2122, 0x2122, 0x2139, 0x2139, 0x2194, 0x21AA, 0x231A, 0x231B,
            0x2328, 0x2328, 0x2600, 0x27BF, 0x2934, 0x2935, 0x2B05, 0x2B07,
            0x2B1B, 0x2B1C, 0x2B50, 0x2B50, 0x2B55, 0x2B55, 0x3030, 0x3030,
            0x303D, 0x303D, 0x3297, 0x3297, 0x3299, 0x3299, 0x1F000, 0x1F0FF,
            0x1F100, 0x1F1AD, 0x1F200, 0x1F2FF, 0x1F300, 0x1F5FF, 0x1F600, 0x1F64F,
            0x1F680, 0x1F6FF, 0x1F700, 0x1F77F, 0x1F780, 0x1F7FF, 0x1F800, 0x1F8FF,
            0x1F900, 0x1F9FF, 0x1FA00, 0x1FAFF, 0x1FC00, 0x1FFFD
        };

        /// <summary>
        /// The Unicode code point for the Zero Width Joiner, used to fuse emoji sequences.
        /// </summary>
        public const int ZeroWidthJoiner = 0x200D;

        /// <summary>
        /// The Unicode code point for the emoji-presentation variation selector (VS16).
        /// </summary>
        public const int VariationSelector16 = 0xFE0F;

        /// <summary>
        /// Returns the number of terminal columns occupied by a single Unicode scalar value.
        /// </summary>
        /// <param name="codePoint">The Unicode scalar value (0 through 0x10FFFF).</param>
        /// <returns>
        /// 0 for combining, zero-width, or non-printable control scalars; 2 for East Asian wide,
        /// fullwidth, and emoji scalars; otherwise 1.
        /// </returns>
        public static int CodePointWidth(int codePoint)
        {
            ...
            // C0 and C1 control ranges occupy no visible column in a cell grid.
            if (codePoint < 0x20 || (codePoint >= 0x7F && codePoint < 0xA0))
                return 0;

            if (InTable(_ZeroWidth, codePoint))
                return 0;

            if (InTable(_Wide, codePoint))
                return 2;
        ...
        /// <summary>
        /// Determines whether a scalar value is an extended pictographic (emoji) code point.
        /// </summary>
        /// <param name="codePoint">The Unicode scalar value.</param>
        /// <returns><c>true</c> when the scalar is emoji; otherwise <c>false</c>.</returns>
        public static bool IsExtendedPictographic(int codePoint)
        {
            return InTable(_Pictographic, codePoint);
        }

        /// <summary>
        /// Determines whether a scalar value is a zero-width combining or format code point.
        /// </summary>
        /// <param name="codePoint">The Unicode scalar value.</param>
        /// <returns><c>true</c> when the scalar has zero width; otherwise <c>false</c>.</returns>
        public static bool IsZeroWidth(int codePoint)
        {
            if (codePoint < 0x20 || (codePoint >= 0x7F && codePoint < 0xA0))
                return true;

            return InTable(_ZeroWidth, codePoint);
        }

        /// <summary>
        /// Determines whether a scalar value is a regional indicator symbol, two of which combine
        /// into a flag grapheme.
        /// </summary>
        /// <param name="codePoint">The Unicode scalar value.</param>
        /// <returns><c>true</c> when the scalar is a regional indicator; otherwise <c>false</c>.</returns>
        public static bool IsRegionalIndicator(int codePoint)
        {
            return codePoint >= 0x1F1E6 && codePoint <= 0x1F1FF;
        }

        private static bool InTable(int[] table, int codePoint)
        {
            int low = 0;
            int high = (table.Length / 2) - 1;

            while (low <= high)
            {
                int mid = (low + high) / 2;
                int lo = table[mid * 2];
                int hi = table[(mid * 2) + 1];
