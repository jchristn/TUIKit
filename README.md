...
> **v0.6.0 — Alpha.** An early public preview. The API and capabilities are subject to change. It is usable and extensively tested, but treat it as pre-1.0: pin your version and expect breaking changes between minor releases until it stabilizes. The 0.6 line adds **per-region background colors** (an explicit color or a theme role, so a sidebar can be tinted and restyled by the theme) and a batch of horizontal components: a `DialogModal` base that removes the box-drawing boilerplate every dialog repeats, a `CheckList<T>` widget and `MultiSelectModal<T>`, a `DefinitionList` status panel, an `ActivityIndicator`, a `StreamingTranscript` pane projector, and small utilities (`HintText`, `ColumnFormatter`, a `Rule` divider, and a `SubmitKeyResolver`). See the [changelog](CHANGELOG.md#060---2026-08-13).
...
```xml
<PackageReference Include="TUIKit" Version="0.7.0" />
```
